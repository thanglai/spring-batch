...
@Configuration
@ConditionalOnProperty(value = "async", havingValue = "true")
@XSlf4j
public class CsvAsyncProcessConfiguration {
  ...
  @Bean
  @StepScope
  SynchronizedItemStreamReader<Item> syncCsvItemReader(FlatFileItemReader<Item> csvItemReader) throws Exception {
    var reader = new SynchronizedItemStreamReader<Item>();
  ...
  @StepScope
  @Primary
  AsyncItemWriter<Item> consoleItemWriter() {
    var writer = new AsyncItemWriter<Item>();
    writer.setDelegate(new CsvItemWriter());
    return writer;
  }

  @Bean
  @StepScope
  AsyncItemProcessor<Item, Item> asyncCsvItemProcessor() throws Exception {
    var processor = new AsyncItemProcessor<Item, Item>();
    processor.setDelegate(csvItemProcessor());
  ...
  @Bean
  Step step1(JobRepository jobRepository, PlatformTransactionManager transactionManager, SynchronizedItemStreamReader<Item> csvItemReader,
      AsyncItemProcessor<Item, Item> csvItemProcessor, AsyncItemWriter<Item> csvItemWriter) throws IOException {
    // @formatter:off
        return new StepBuilder("step1", jobRepository)
          .<Item, Future<Item>> chunk(10, transactionManager)
          .faultTolerant()
          .skipLimit(1)
